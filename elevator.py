...
class Elevator:
    ...
    def setgoingDown(self,Bool):            self.goingDown=Bool
    def getgoingDown(self):                 return self.goingDown
    ...
    def drawInfo(self):
        ...
        for i in self.RenderedText:
            ...
            distance += 15
    def addToStopList3(self, floorNumber):
        ...
        if self.Status == ["DOWN"]: self.StopList = sorted(LessThenCurrentFloor, reverse = False)    + sorted(MoreThenCurrentFloor)
        elif self.Status == ["UP"]: self.StopList = sorted(MoreThenCurrentFloor)                     + sorted(LessThenCurrentFloor)   
    def addToStopList2(self, floorNumber):
        if floorNumber not in self.StopList:
            self.StopList.append(floorNumber)
            if self.Status == ["IDLE"] and self.StopList[0] > self.CurrentFloor:    self.Status = ["UP"]
            elif self.Status == ["IDLE"]:                                           self.Status = ["DOWN"]
        ...
        if self.Status == ["DOWN"]: self.StopList = sorted(LessThenCurrentFloor, reverse = False)    + sorted(MoreThenCurrentFloor)
        elif self.Status == ["UP"]: self.StopList = sorted(MoreThenCurrentFloor)   
